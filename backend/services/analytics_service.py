...
def _yf_ticker_for_asset(asset) -> str | None:
    ...
    if asset.category not in (AssetCategory.STOCK, AssetCategory.CRYPTO) or not asset.ticker:
        return None
    t = asset.ticker
    if t.isdigit() and len(t) == 4:
        t = f"{t}.TW"
    if ("Crypto" in (asset.sub_category or "") or asset.category == AssetCategory.CRYPTO) and "-" not in t:
        t = f"{t}-USD"
    return t
...
def get_net_worth_history(db: Session, range_str: str = "30d") -> list[dict]:
    ...
    while current_date <= today:
        ...
        for asset in assets:
            ...
            elif asset.category in (AssetCategory.STOCK, AssetCategory.CRYPTO):
